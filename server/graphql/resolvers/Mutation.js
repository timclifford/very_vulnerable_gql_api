...
const Mutation = {
  ...
  updateMedicalRecord: combineResolvers(isAuthenticated,
    async (parent, {
      input: { patient, patch }
  ...
  addPatient: combineResolvers(
    ...
    async (parent, {
      _id, name, address, phone_number, doctor, practice, appointments
    ...
    async (parent, {
      ...
      if (_id) {
        try {
          ...
          throw new Error(`${error.message} - ${error.value}`);
        }

      }
      else if (name) {
        const { _id } = await Patient.findOne(JSON.parse(name));
        ...
        const addedPatient = await Patient.findById({ _id });
        return addedPatient;
      }
    }
  ),
