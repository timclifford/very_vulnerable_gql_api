...
const Query = {
  ...
  me: async (parent, args, { Practice, User, user }, req) => {
    ...
    const { practice } = userObj._doc || null;
  ...
  user: async (parent, args, { User, Practice, user }, req) => {
    let userObj = {};
    if (args._id) {
    ...
    let practiceObj = {};
    if (userObj.practice) {
      practiceObj = await Practice.findById(userObj.practice);
      return { ...userObj._doc, practice: practiceObj };
    }
    else {
      return { ...userObj._doc };
    }
